import { ChartDataPoint, PredictionResult, ProfitPrediction } from './types';

export function predictWithSMA( data: ChartDataPoint[], periods: number = 5, futureSteps: number = 5): PredictionResult[] {
    if (data.length < periods) {
        console.warn('Not enough data for SMA prediction');
        return [];
    }
...
export function predictWithEMA( data: ChartDataPoint[], alpha: number = 0.3, futureSteps: number = 5 ): PredictionResult[] {
    if (data.length < 2) {
        console.warn('Not enough data for EMA prediction');
        return [];
    }
...
export function predictWithLinearRegression( data: ChartDataPoint[], futureSteps: number = 5 ): PredictionResult[] {
    if (data.length < 3) {
        console.warn('Not enough data for linear regression prediction');
        return [];
    }
    ...
    for (let i = 1; i <= futureSteps; i++) {
        ...
        const marginOfError = 1.96 * stdError * Math.sqrt(
            1 + 1 / n + Math.pow(futureIndex - sumX / n, 2) / (sumX2 - sumX * sumX / n)
        );
...
export function calculatePredictionAccuracy(predictions: PredictionResult[], actualData: ChartDataPoint[]): { mae: number; mape: number; accuracy: number; } {
    ...
    predictions.forEach((pred, idx) => {
        const actual = actualData.find(d => d.timestamp === pred.timestamp);
        if (actual) {
            const error = Math.abs(actual.spread - pred.predictedSpread);
            totalError += error;
            totalPercentError += (error / Math.abs(actual.spread)) * 100;
            matchCount++;
        }
    });
    ...
    const mae = totalError / matchCount;
    const mape = totalPercentError / matchCount;
    const accuracy = Math.max(0, 100 - mape);

    return { mae, mape, accuracy };
}

export function detectTrend(data: ChartDataPoint[]): 'BULLISH' | 'BEARISH' | 'NEUTRAL' {
    if (data.length < 5) return 'NEUTRAL';

    const recentSpreads = data.slice(-5).map(d => d.spread);
    const firstHalf = recentSpreads.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
    const secondHalf = recentSpreads.slice(2, 5).reduce((a, b) => a + b, 0) / 3;
...
export function calculateVolatility(data: ChartDataPoint[], periods: number = 10): number {
    if (data.length < periods) return 0;

    const recentSpreads = data.slice(-periods).map(d => d.spread);
    const mean = recentSpreads.reduce((a, b) => a + b, 0) / periods;
    const variance = recentSpreads.reduce((sum, val) =>
        sum + Math.pow(val - mean, 2), 0) / periods;
